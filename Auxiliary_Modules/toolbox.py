...
def get_root_path_project():
    """
    Функция возвращает абсолютный путь корня проекта по файлу маркеру, файл маркер должен быть в корне проекта.
    Returns: str
    """
    current_directory = os.path.abspath(os.path.dirname(__file__))  # Получаем текущую директорию скрипта
    while True:
...
#     def __init__(self, limit=25, ssl=True, total=320, sock_connect=160, sock_read=160, trust_env=False, logger=None):
#         """
